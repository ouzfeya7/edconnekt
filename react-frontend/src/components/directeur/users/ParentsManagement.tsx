...
const ParentsManagement = () => {
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow">
      <h2 className="text-xl font-semibold mb-4">{t('parents_list', 'Liste des Parents')}</h2>
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('name', 'Nom')}</th>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('child', 'Enfant')}</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {mockParents.map((parent) => (
            <tr key={parent.id}>
              ...
              <td className="px-6 py-4 whitespace-nowrap">{parent.child}</td>
