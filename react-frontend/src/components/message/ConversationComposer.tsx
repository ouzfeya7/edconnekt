...
const ConversationComposer: React.FC<Props> = ({ conversationId }) => {
  ...
  return (
    <div className="border-t border-gray-200 p-2 md:p-3 bg-[#f0f2f5] flex items-center gap-2">
      <input
        value={text}
        ...
        className="flex-1 border border-gray-300 rounded-full px-3 md:px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-400"
      />
      <label className="px-1 md:px-2 py-2 text-gray-600 hover:text-gray-800 cursor-pointer">
        📎
        <input
          type="file"
          className="hidden"
      ...
      <button onClick={handleSend} className="px-2 md:px-3 py-2 bg-green-400 text-white rounded-full hover:bg-green-500">
        <Send size={16} />
      </button>
    </div>
  );
