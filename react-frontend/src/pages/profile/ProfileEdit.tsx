...
const ProfileEdit: React.FC<ProfileEditProps> = ({ currentUser, onCancel, onSave }) => {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col items-center mb-8">
            <div className="relative w-24 h-24 rounded-full bg-gray-100 flex items-center justify-center mb-2">
              {formData.imageUrl ? (
                <img src={formData.imageUrl} alt="Profil" className="w-full h-full rounded-full object-cover" />
              ) : (
            ...
            <label htmlFor="photo-upload" className="text-sm text-blue-600 hover:text-blue-700 cursor-pointer font-semibold">
              Changer la photo
              <input id="photo-upload" name="imageUrl" type="file" className="hidden" onChange={handlePhotoUpload} accept="image/*" />
            </label>
          </div>
