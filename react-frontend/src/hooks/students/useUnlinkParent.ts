...
export function useUnlinkParent() {
  ...
  return useMutation<void, Error, { studentId: string; parentId: string }>({
    mutationKey: ['student:unlink-parent'],
    mutationFn: async ({ studentId, parentId }) => {
      await studentsApi.unlinkParentFromStudentApiStudentsStudentIdParentsParentIdDelete(studentId, parentId);
    },
    onSuccess: (_d, vars) => {
