...
const CircularProgress: React.FC<{ percentage: number, colorClass: string }> = ({ percentage, colorClass }) => {
  ...
  return (
    <div className="relative w-16 h-16 flex items-center justify-center">
      ...
      {percentage !== undefined && <span className={`absolute text-sm font-semibold ${colorClass}`}>{percentage}%</span>}
    </div>
  );
};
